...
// we can either do this, by normal function type, returning void, accepting as head
void take_input(node* head)
{

}

// this is in new style, which returns node* type
node* take_input2()
{
    int d;
    cin >> d;
...
int main ()
{
    ...
    printLL(head);
    // since we are always adding at head, the linkedlist we get will be in the reverse order

    // TAKING INPUT FROM A FILE!!!
    /*
        You can also take inputs from a file rather than giving it into the terminal by:
        creating a file eg. "input.txt" in the same folder
        then running the code but with the argument " < input.txt" this will give input.txt as an input
